...
import { z } from "zod";
import { adminAuditEvents, adminCredentials, businesses, businessInvitations, businessMembers, users } from "../drizzle/schema.js";
...
export async function createBusinessInvitation(actorUserId, input) {
  const db = await getDb();
  if (!db) {
...
export async function revokeBusinessInvitation(actorUserId, invitationId) {
  const db = await getDb();
  if (!db) return { revoked: true };
...
export async function acceptBusinessInvitation(user, token) {
  const db = await getDb();
  if (!db) return { accepted: true, businessId: 1 };
  ...
  return { accepted: true, businessId: current.businessId };
}
...
export async function searchAdminAuditLog(input) {
  const db = await getDb();
  if (!db) {
    return [
      {
