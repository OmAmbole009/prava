...
import { actionItems, auditEvents, businessMembers, documentExtractions, documents, gstPreparations, operationalTasks, reconciliationItems, taskRequirements } from "../drizzle/schema";
...
vi.mock("./storage", () => ({ storageGetSignedUrl: mocks.storageGetSignedUrl, storagePut: vi.fn() }));
vi.mock("./entitlements", () => ({ checkFeatureEntitlement: mocks.checkFeatureEntitlement, consumeUsage: mocks.consumeUsage }));

import { prepareGstReturn, reconcileGstTask, reviewDocumentForUser } from "./operations";
...
function createWorkflowHarness() {
  ...
  const resolveRows = (table: unknown, fields?: Record<string, unknown>) => {
    if (table === documents) {
  ...
  const db: any = {
    select: vi.fn((fields?: Record<string, unknown>) => {
      ...
      const builder: any = {
        ...
        then(resolve: (value: unknown) => unknown) { return Promise.resolve(resolveRows(table, fields)).then(resolve); },
      };
      return builder;
    }),
    update: vi.fn((table: unknown) => ({
      set(values: Record<string, unknown>) {
        return { where: async () => {
          ...
          if (table === taskRequirements) state.requirements.forEach((item: any) => Object.assign(item, values));
          if (table === actionItems) state.actions.forEach((item: any) => Object.assign(item, values));
    ...
    insert: vi.fn((table: unknown) => ({
      values(values: any) {
        if (table === gstPreparations) {
          ...
          return { onDuplicateKeyUpdate: async ({ set }: { set: Record<string, unknown> }) => Object.assign(state.preparation, set) };
        }
...
describe("non-production document-to-GST service harness", () => {
  it("runs reviewed document approval through task preparation and reconciliation persistence", async () => {
    ...
    expect(state.requirements.every((item: { status: string }) => item.status === "complete")).toBe(true);
