...
import { checkFeatureEntitlement, consumeUsage } from "./entitlements";
import { invokeLLM } from "./_core/llm";
import { storageGetSignedUrl, storagePut } from "./storage";
import { notifyApproval } from "./gstAdmin";
...
export function reviewActionOutcome(decision: "approve" | "save_for_review") {
  return decision === "approve" ? "resolved" as const : "open" as const;
}

export function authorizedSubmissionGate(task: { type: string; status: string; requiresProfessionalReview: number }, preparation: { status: string } | null) {
  if (task.type !== "gst_return_preparation") return { allowed: false, reason: "Only GST return preparation tasks can enter an authorized submission workflow." } as const;
  if (task.status !== "prepared" || preparation?.status !== "prepared") return { allowed: false, reason: "Resolve required evidence and review items before requesting authorized submission." } as const;
  if (task.requiresProfessionalReview) return { allowed: false, reason: "Complete the requested professional review before seeking authorized submission." } as const;
  return { allowed: true } as const;
}

export function canIndependentlyApproveSubmission(input: { workspaceRole: string; requestedByUserId: number; reviewerUserId: number; status: string }) {
  if (input.workspaceRole !== "admin") return { allowed: false, reason: "A workspace administrator must approve an authorized submission request." } as const;
  if (input.requestedByUserId === input.reviewerUserId) return { allowed: false, reason: "The requester cannot approve their own submission request." } as const;
  if (input.status !== "awaiting_review") return { allowed: false, reason: "This submission request is no longer awaiting review." } as const;
  return { allowed: true } as const;
}

export function normalizeProviderFailureCode(value: string) {
  return value.trim().replace(/[^a-zA-Z0-9._-]/g, "_").slice(0, 128);
}

export async function resolveLinkedDocumentReviewActions(
  db: { update: (table: typeof actionItems) => any },
  documentId: number,
  decision: "approve" | "save_for_review",
) {
...
function taskTitle(type: (typeof taskTypes)[number], period?: string) {
...
function requirementKeyForDocument(documentType: string, invoiceType?: string | null) {
...
export function calculateGstPreparation(rows: Array<{ invoiceType: string | null; taxableValueMinor: number | null; cgstMinor: number | null; sgstMinor: number | null; igstMinor: number | null; totalMinor: number | null; status: string }>) {
  let salesMinor = 0;
...
async function requireMember(userId: number, businessId: number) {
...
async function audit(businessId: number, actorUserId: number, action: string, entityType: string, entityId?: string, taskId?: number, metadata?: Record<string, unknown>) {
  const db = await getDb();
  if (!db) return;
...
export async function createOperationalTask(userId: number, input: z.infer<typeof createTaskSchema>) {
  ...
  const requirements = input.type === "gst_return_preparation"
    ...
    : [["business_profile", "Business profile", "complete", "Workspace profile is available."]] as const;
...
export async function listTasksForUser(userId: number, businessId: number) {
    ...
    return [
      {
        ...
        businessId,
        createdByUserId: userId,
        type: "gst_return_preparation" as const,
      ...
      },
      {
        ...
        businessId,
        createdByUserId: userId,
        type: "cash_reconciliation" as const,
...
export async function getActionCenter(userId: number, businessId: number) {
  await requireMember(userId, businessId);
  const db = await getDb();
  if (!db) {
    return [
...
export async function resolveActionItem(userId: number, actionId: number, resolution: "resolved" | "dismissed") {
  const db = await getDb();
...
export async function resolveTaskRequirement(userId: number, input: z.infer<typeof requirementResolutionSchema>) {
...
async function extractInvoice(storageKey: string, mimeType: string): Promise<Extraction> {
...
export async function uploadAndProcessDocument(userId: number, input: z.infer<typeof uploadDocumentSchema>) {
  ...
  if (input.documentType === "bank_statement") {
    ...
    return { documentId, status: "uploaded" as const, reviewReasons: ["Bank statement stored as source evidence. Transaction-level extraction has not been activated."] };
  }
  try {
    const raw = await extractInvoice(stored.key, input.mimeType);
    const validated = validateExtraction(raw);
    const extractionStatus = validated.reasons.length ? "needs_review" : "extracted" as const;
    ...
    return { documentId, status: "needs_review" as const, reviewReasons: ["Automated extraction could not be completed."] };
  }
}

export async function listDocumentsForUser(userId: number, businessId: number, taskId?: number) {
  await requireMember(userId, businessId);
  const db = await getDb();
  if (!db) {
    return [
...
export async function persistReviewedDocumentState(input: {
  ...
  documentType: string;
  decision: "approve" | "save_for_review";
  previousConfidenceBps: number | null;
  raw: Extraction;
  validated: ReturnType<typeof validateExtraction>;
}) {
  const status = input.decision === "approve" ? "extracted" as const : "needs_review" as const;
...
export async function reviewDocumentForUser(userId: number, input: z.infer<typeof reviewDocumentSchema>) {
...
export async function prepareGstReturn(userId: number, taskId: number) {
...
export async function requestProfessionalReview(userId: number, taskId: number, note?: string) {
  const task = await getTaskForUser(userId, taskId);
  if (!task) throw new Error("Task not found.");
  const db = await getDb();
 ...
 */
export async function markGstSubmissionPending(userId: number, taskId: number) {
  void userId;
  void taskId;
...
export async function requestAuthorizedGstSubmission(userId: number, input: z.infer<typeof authorizedSubmissionRequestSchema>) {
  const task = await getTaskForUser(userId, input.taskId);
  if (!task) throw new Error("Task not found.");
...
export async function approveAuthorizedGstSubmission(userId: number, input: z.infer<typeof authorizedSubmissionApprovalSchema>) {
  const task = await getTaskForUser(userId, input.taskId);
  if (!task) throw new Error("Task not found.");
  const db = await getDb();
  if (!db) throw new Error("GST submission storage is unavailable.");
 ...
 */
export async function beginAuthorizedGstProviderDispatch(actorUserId: number, taskId: number, providerName: string) {
 ...
 */
export async function recordAuthorizedGstProviderFailure(actorUserId: number, taskId: number, failureCode: string) {
...
export async function recordOfficialGstSubmission(actorUserId: number, taskId: number, officialReference: string) {
...
export function buildGstReconciliationItems(rows: ReconciliationSource[], missingRequirements: Array<{ label: string }>) {
  const seen = new Set<string>();
  const items: Array<{ documentId: number | null; status: "matched" | "missing" | "duplicate" | "mismatch" | "needs_review"; itemType: string; reference: string; reason: string }> = [];
  for (const row of rows) {
    const reference = row.invoiceNumber || `Document ${row.documentId}`;
    let reviewReasons: string[] = [];
    try { reviewReasons = (JSON.parse(row.extractedData) as { reviewReasons?: string[] }).reviewReasons ?? []; } catch { reviewReasons = ["Extraction payload could not be read."]; }
...
type WorkflowDocumentSource = ReconciliationSource & {
  invoiceType: string | null;
  taxableValueMinor: number | null;
...
export function deriveDocumentToGstWorkflow(rows: WorkflowDocumentSource[], requirements: Array<{ status: string; label: string }>) {
  ...
  const preparationStatus = requiresReview ? "needs_review" as const : "prepared" as const;
  ...
  const reconciliationStatus = reconciliationItems.some(item => item.status !== "matched") ? "needs_review" as const : "ready_for_review" as const;
  return { preparation, incompleteRequirements, requiresReview, preparationStatus, reconciliationItems, reconciliationStatus };
}

type CashIntakeSource = {
  documentId: number;
  originalName: string;
  documentType: string;
  status: string;
 ...
 */
export function deriveCashIntakeReconciliation(rows: CashIntakeSource[], requirements: Array<{ status: string; label: string }>) {
  const statements = rows.filter(row => row.documentType === "bank_statement");
  const items: Array<{ documentId: number | null; status: "missing" | "needs_review"; itemType: string; reference: string; reason: string }> = [];
  ...
  return { items, reconciliationStatus: statements.length ? "needs_review" as const : "collecting" as const };
}

export async function prepareCashReconciliation(userId: number, taskId: number) {
  const task = await getTaskForUser(userId, taskId);
...
export async function reconcileGstTask(userId: number, taskId: number) {
  const task = await getTaskForUser(userId, taskId);
...
export async function listReconciliationForUser(userId: number, taskId: number) {
  const task = await getTaskForUser(userId, taskId);
  if (!task) throw new Error("Task not found.");
  const db = await getDb();
  if (!db) {
...
export async function resolveReconciliationItem(userId: number, input: z.infer<typeof reconciliationResolutionSchema>) {
  const db = await getDb();
  if (!db) throw new Error("Reconciliation storage is unavailable.");
