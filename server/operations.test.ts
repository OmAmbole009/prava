...
import { authorizedSubmissionGate, buildGstReconciliationItems, calculateGstPreparation, canIndependentlyApproveSubmission, deriveCashIntakeReconciliation, deriveDocumentToGstWorkflow, normalizeProviderFailureCode, persistReviewedDocumentState, resolveLinkedDocumentReviewActions, reviewActionOutcome, validateExtraction } from "./operations";
...
describe("financial-operations deterministic safeguards", () => {
  ...
  it("persists approval through document, requirement, action, and downstream GST workflow state", async () => {
    ...
    const set = vi.fn(() => ({ where }));
    const update = vi.fn(() => ({ set }));
    const raw = { vendorName: "Supplier", gstin: "27ABCDE1234F1Z5", invoiceNumber: "P-001", invoiceDate: "2026-08-10", taxableValue: "100.00", cgst: "9.00", sgst: "9.00", igst: "0", total: "118.00", placeOfSupply: "Maharashtra", invoiceType: "purchase" as const, reviewReasons: [] };
    const persisted = await persistReviewedDocumentState({ db: { update } as never, documentId: 7, taskId: 3, documentType: "invoice", decision: "approve", previousConfidenceBps: 5000, raw, validated: validateExtraction(raw) });
