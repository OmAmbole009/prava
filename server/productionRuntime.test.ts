...
import { productionResponseHeaders, publicServerError, safeRequestId } from "./_core/productionRuntime";
...
describe("production runtime safeguards", () => {
  ...
  it("applies non-breaking security headers and enables HSTS only for secure requests", () => {
    const insecure = productionResponseHeaders({ protocol: "http", headers: {} } as never);
    const secure = productionResponseHeaders({ protocol: "http", headers: { "x-forwarded-proto": "https" } } as never);
