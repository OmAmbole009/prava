...
import { z } from "zod";
import { adminAuditEvents, integrationSettings } from "../drizzle/schema.js";
import { getDb } from "./db.js";

function state(required) {
  const configured = required.filter(value => Boolean(process.env[value]));
  return {
    status: configured.length === required.length ? "ready" : "not_configured",
...
export const integrationSettingsSchema = z.object({
  ...
  for (const field of ["apiBaseUrl", "webhookUrl"]) {
...
export const integrationTypes = ["email", "gst_provider", "razorpay", "upi"];
...
export async function saveIntegrationSettings(actorUserId, input) {
  const db = await getDb();
  if (!db) {
